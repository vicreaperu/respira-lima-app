...
/////////                                                       // --------- END STREET NAME--------
  ...
  */
  void add_point(PointMi current_point) {
    ...
    this.valid_points += 1;
    int current_point_number = current_point.pointNumber;

    //obtener el previous point
    PointMi? previous_point = PointMi.findByNumber(current_point_number-1);

    //verify that previous and current point mus be valid
    if (_is_valid_point(current_point) &&
        _is_valid_point(previous_point)) {
      Segment current_segment =
          new Segment(previous_point!, current_point);
...
class RouteReporter {
  ...
  RouteReporter(String route_id, PM25 _pm25) {
    this.route_id = route_id;
    this.route =
        new RouteMi(route_id); //permite hacer consultas de parametros hacia db
    this._pm25 = _pm25;
  }

  Map<String, dynamic> get_temporary_report(int current_point_number) {

    //se debe obtener el punto actual
    PointMi? current_point = PointMi.findByNumber(current_point_number);

    if (!this.route._is_valid_point(current_point)) return {'error': 100};

    double current_pm25 = current_point!.pm25;
    ...
    Map<String, dynamic> temporary_report = {
      ...
      "street_name": current_point.streetName
    };
